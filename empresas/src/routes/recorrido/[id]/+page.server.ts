...
export const load = async ({ cookies, params }) => {
    ...
    const id_recorrido = params.id;
    req = await fetch(`${API_URL}/empresa/${id}/recorrido/${id_recorrido}`);
    if (!req.ok) {
        throw error(404, "Not Found");
    };
    const recorrido_db: Recorrido = await req.json();
    const recorrido: { [key: string]: string | number } = recorrido_db;
    for (let i = 0; i < 7; i++) {
        const dia = recorrido_db.dias[i];
        recorrido[i] = dia;
    };
    return { terminales, recorrido };
}
